...
def move_and_rename(targetPath, framePath, newPath, processed):
    ...
    i = 0
    for dataname in data_names:#the 'data_name' in targets.crv
        for filename in file_names:#the raw_frames
            if processed:
                idx = filename
                ...
                x=int(dataname.lstrip('frame_')) 
                if int(dataname.lstrip('frame_')) == int(filename.strip('frame_.png')):
                    #shutil.copy(f'{framePath}/{filename}', f'{newPath}/image_{i}.png')
                    shutil.move(f'{framePath}/{filename}', f'{newPath}/image_{x}.png')
                    i=i+1 
