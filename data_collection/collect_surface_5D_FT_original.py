...
def collect(target_df, dataPath, resume_from, sleep_time, i):
    ...
    for _, row in target_df.iloc[resume_from:].iterrows():
        try:
            ...
            print(f'pose for frame_{i} = {pose}')
            # Add extra depth to account additional geomrtry in Rx direction, adding more depth from 0 deg to 22 deg
            add_tap_depth = max((35 - abs(pose[3]))/35 * 2.75, 0.0)#2.75
            # check if pose is zero, raise tip more to avoid contact
            if not any(pose):
                add_tap_depth-= 2
            #add_tap_depth=0
...
tcp = 65#65            # DONT CHANGE THIS VALUE, CHANgE "offset" INSTEAD!!!!
offset = -5#-5         # Change this value to adjust the height of the tcp. negative value moves tcp up 
...
work_frame = (498, -111, 60-offset, -180, 0, -90) #61   y#-111   z#low the height lower, Bigger Z would be safer
tcp_x_offset = -0.8  #2.5       # Change this value to adjust the tcp in the forward backward direction, negative value moves tcp away the robot
...
if resume_from == 0:
    resume = False
    poses_rng = [[0, 0, 0.5,-34,-34,0], [0, 0, 5,34,34, 0]]    # pose ranges (min values, max values)
    #poses_rng = [[0, 0, 0.5,-0, 0, 0], [0, 0, 0.5,-0,0, 0]]    # pose ranges (min values, max values)
